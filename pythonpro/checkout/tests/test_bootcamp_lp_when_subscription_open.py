...
def test_discount_datetime_on_context(resp_no_user_after_discount):
    assert resp_no_user_after_discount.context['discount_datetime_limit'] == facade.discount_50_percent_datetime_limit
...
def resp_no_user_after_launch_with_debug(client, tag_as_mock, freezer):
    ...
    return client.get(reverse('checkout:bootcamp_lp'), data={'debug': 'true'})
...
@pytest.fixture
def resp_no_user_before_50_discount(client, tag_as_mock, freezer):
    freezer.move_to(facade.discount_50_percent_datetime_limit - timedelta(seconds=1))
    return client.get(reverse('checkout:bootcamp_lp'))


def test_no_user_with_first_day_discount(resp_no_user_before_50_discount):
    assert resp_no_user_before_50_discount.context['payment_item_config'].slug == 'bootcamp-50-discount'
    assert resp_no_user_before_50_discount.context['client_discount'] == 0
    assert resp_no_user_before_50_discount.context['first_day_discount'] == 100000
    assert resp_no_user_before_50_discount.context['has_first_day_discount'] is True
    assert resp_no_user_before_50_discount.context['has_client_discount'] is False
    dj_assert_contains(resp_no_user_before_50_discount, 'R$ 99,63')
    discount_video = 'https://www.youtube.com/embed/0rzm6NjyoSw'
    dj_assert_contains(resp_no_user_before_50_discount, discount_video)
...
def resp_not_level_two_after_discount(client_with_not_level_one_roles, logged_user, tag_as_mock, freezer):
    ...
    return client_with_not_level_one_roles.get(reverse('checkout:bootcamp_lp'))
...
def test_not_level_two_without_discount(resp_not_level_two_after_discount):
    test_no_user_no_discount(resp_not_level_two_after_discount)


def test_not_level_two_with_first_day_discount(client_with_not_level_one_roles, logged_user, tag_as_mock, freezer):
    freezer.move_to(facade.discount_50_percent_datetime_limit - timedelta(seconds=1))
    # freezer for some reason logout user, so must be logged again
    client_with_not_level_one_roles.force_login(logged_user)
    resp = client_with_not_level_one_roles.get(reverse('checkout:bootcamp_lp'))
    test_no_user_with_first_day_discount(resp)


def test_webdev_without_first_day_discount(client_with_webdev, logged_user, tag_as_mock, freezer):
    freezer.move_to(facade.discount_50_percent_datetime_limit + timedelta(seconds=1))
    client_with_webdev.force_login(logged_user)  # freezer for some reason logout user, so must be logged again
    resp = client_with_webdev.get(reverse('checkout:bootcamp_lp'))
...
def test_webdev_with_first_day_discount(client_with_webdev, logged_user, tag_as_mock, freezer):
    freezer.move_to(facade.discount_50_percent_datetime_limit - timedelta(seconds=1))
    client_with_webdev.force_login(logged_user)  # freezer for some reason logout user, so must be logged again
    resp = client_with_webdev.get(reverse('checkout:bootcamp_lp'))
