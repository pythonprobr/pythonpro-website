...
@pytest.fixture
def valid_resp(cohort, client, logged_user, valid_signature, transaction_response, create_or_update_member, mailoutbox,
               tag_as_mock):
    transaction_response['items'][0]['id'] = f'membership-{logged_user.id}'
...
def _generate_response(client, user, valid_signature, raw_post):
    return client.generic(
        'POST',
        reverse('payments:membership_notification', kwargs={'user_id': user.id}),
        raw_post,
        secure=True,
...
@pytest.fixture
def valid_resp_anonymous(cohort, client, anonymous_user, valid_signature, transaction_response, create_or_update_member,
                         mailoutbox, tag_as_mock):
    transaction_response['items'][0]['id'] = 'membership-'
    with responses.RequestsMock(assert_all_requests_are_fired=True) as r:
        r.add(r.GET, 'https://api.pagar.me/1/transactions/1396639', json=transaction_response, status=200)
        yield client.generic(
            'POST',
            reverse('payments:membership_anonymous_notification'),
            valid_raw_post,
            secure=True,
            content_type='application/x-www-form-urlencoded',
            HTTP_X_HUB_SIGNATURE=valid_signature
...
def test_anonymous_user_promoted_to_member(valid_resp_anonymous, anonymous_user):
    ...
    assert not has_role(anonymous_user, 'client')


def test_user_promoted_to_member(valid_resp, logged_user):
...
def test_tampered_post(client, logged_user, valid_signature, transaction_response, create_or_update_member, mailoutbox):
    ...
    assert_no_action_taken(create_or_update_member, logged_user, mailoutbox)


def assert_no_action_taken(create_or_update_member, logged_user, mailoutbox):
...
def test_item_id(client, logged_user, valid_signature, transaction_response, create_or_update_member, mailoutbox):
    transaction_response['items'][0]['id'] = f'membership-{logged_user.id + 1}'
...
def test_no_subscription_handling(client, logged_user, valid_signature, transaction_response, create_or_update_member,
                                  mailoutbox, mocker):
    ...
    _generate_response(
        ...
        logged_user,
        valid_signature,
        template.format(price=MEMBERSHIP_PRICE, object='subscription', current_status='paid'))
    assert_no_action_taken(create_or_update_member, logged_user, mailoutbox)


def test_no_handling_for_unpaid(client, logged_user, valid_signature, transaction_response, create_or_update_member,
                                mailoutbox, mocker):
    mocker.patch('pythonpro.payments.facade._pagarme.postback.validate').return_value = True
    _generate_response(
        client,
        logged_user,
        valid_signature,
        template.format(price=MEMBERSHIP_PRICE, object='subscription', current_status='waiting_paiment'))
    assert_no_action_taken(create_or_update_member, logged_user, mailoutbox)
...
template = (  # See https://docs.pagar.me/docs/gerenciando-postbacks
    ...
    'transaction%5Bpaid_amount%5D={price}&'
...
valid_raw_post = template.format(price=MEMBERSHIP_PRICE, object='transaction', current_status='paid')
...
def transaction_response():
    return {
        ...
        'date_created': '2019-05-28T21:31:38.391Z', 'date_updated': '2019-05-28T21:31:40.601Z',
        'amount': MEMBERSHIP_PRICE,
        'authorized_amount': MEMBERSHIP_PRICE, 'paid_amount': 0, 'refunded_amount': 0, 'installments': 1, 'id': 1396639,
        'cost': 0,
        ...
        'customer': {
            ...
            'object': 'item', 'id': 'membership-2', 'title': 'Curso Python Pro - Turma Bruno Rocha',
            'unit_price': MEMBERSHIP_PRICE, 'quantity': 1, 'category': None, 'tangible': False,
